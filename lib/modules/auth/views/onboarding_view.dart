...
class _OnboardingState extends State<Onboarding> {
  ...
  void buttonHandler() {
    if (isStarted) {
      if (phoneData != null && (phoneData?['isPhoneValid'] ?? false)) {
        context.read<AuthCubit>().checkPhoneExist(phoneData!['phoneNumber']);
      }
      return;
    }
    setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<AuthCubit, AuthState>(
      listener: (context, state) {
        if (state is CheckPhoneExistSuccess) {
          final response = state.response;
          final isNewUser = response.status == 404;
